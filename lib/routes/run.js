...
const JS_FILE_FILTER = new RegExp('\.(js|jsx|ts)$');
const CSS_FILTER = new RegExp('\.(css|scss)$');

const MAX_ROWS_PER_FILE = config.eslint_max_errors_file;
const verifier = util.createAuth('pull_request', ['opened', 'synchronize', 'edited', 'review_requested']);
const APPROVE_PR = 'APPROVE';
const PR_STATUS_APPROVED = 'APPROVED';
...
module.exports = function* (next) {
  ...
  const fetchers = files.map((el) => {
    ...
    if (el.status !== FILE_STATUS_REMOVED) {
      return github.blob({
        repo: head.repo.full_name,
        ref: head.sha,
        path: el.filename,
  ...
  let result = files.map((el, index) => ({
    name: el.filename,
    sha: el.sha,
    content: blobs[index] ? blobs[index].toString() : '',
  }))
  ...
  const botLastReview = find(orderedReviewList, item => item.user.login === config.github_user);

  // If no errors Approve the PR automatically
  if (!result && reportCss.length === 0) {
    this.body = `No problems were found for PR https://github.com/${base.repo.full_name}/pull/${number}`;
    // If already approved return
    if (botLastReview && botLastReview.state === PR_STATUS_APPROVED) {
      return;
    }
    ...
    if (botLastReview && botLastReview.review_id) {
      reviewReq.review_id = botLastReview.id;
    }
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (result[i].errors.length > 0) {
      for (let j = 0; j < result[i].errors.length; j++) {
        ...
        const ruleName = result[i].errors[j].rule;
        const commentBody = `<summary><a href="https://eslint.org/docs/rules/${ruleName}"><b>[${ruleName}]: </b></a>${result[i].errors[j].message}</summary>`;
        const commentReq = {
          repo: base.repo.full_name,
          pr: number,
          commit_id: head.sha,
...
function format(results, filesRange, filesMap) {
  ...
  for (let jj = 0; jj < results.length; jj++) {
    ...
    if (messagesCount > MAX_ROWS_PER_FILE) {
      const errorMsg = `OMITTED: ${messagesCount} found! Too much problems...`;
      detail = formatSimgleRowMessage(errorMsg, results[jj].messages, filesMap[jj]);
    } else {
...
/**
 * Returns an error to comment in the review
 * @param message
 */
function formatSimgleRowMessage(message, errors, fileMap) {
  const error = errors[0];
  const formatMsg = {
    type: 'ERROR',
    rule: '[FIX ME PLEASE]',
    line: fileMap[error.line],
    column: error.column,
    message,
  };
  return formatMsg;
