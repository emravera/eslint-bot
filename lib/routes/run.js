...
module.exports = function* (next) {
  ...
  const fetchRemoteFile = util.fetchFileFromRepo(head.repo.full_name, head.sha);

  // Eslintrc file configuration
 let eslintFilePath = yield fetchRemoteFile(config.eslint_file_path, config.eslint_file_name);
 ...
 // Stylelint file configuration
 let stylelintFilePath = yield fetchRemoteFile(config.stylelint_file_path, config.stylelint_file_name);
  ...
  for (let c = 0; c < cssFiles.length; c++) {
    ...
    styleLintConfig.code = code;

    let resultCSS = yield stylelint.lint(styleLintConfig);
